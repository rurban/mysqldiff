    ...
    for my $table1 (@tables_keys) {
        ...
        if (!$self->{opts}{'refs'}) {
            ...
            if (my $table2 = $self->db2->table_by_name($name)) {
                ...
                my $view_exists = $self->db2->view_by_name($name) ? 1 : 0;
                if ($view_exists) {
                                    ...
                                    if ($size == 1) {
                                        ...
                                        if ($field eq $f_last) {
                                            debug(3, "field '$field' is a last part of composite primary key, ".
                                                  "so when it changed, we must to add primary key then");
            ...
            unless($fields1 && $fields1->{$field}) {
                ...
                if (!$self->{added_pk} && ($field_description =~ /AUTO_INCREMENT/is)) {
                    debug(3, "field $field is auto increment, so it will be added without auto_increment clause ".
                          "and then changed when index will be added");
                    $self->{added_index}{field} = $field;
            ...
            if ($indices2 && $indices2->{$index}) {
                ...
                if ($auto) {
                    ...
                    if ($auto_increment_check && $self->{dropped_columns}{$auto}) {
                        debug(3, "Index $auto_index_name was not added to temporary indexes, ".
                              "because of column will be dropped");
                    }
                    else {
                        if (!$self->{temporary_indexes}{$auto_index_name}) {
                            debug(3, "Index $auto_index_name was added to temporary indexes, ".
                                  "because of column will not be dropped and is auto increment")
                    ...
                    for my $index_part (keys %$index_parts) {
                        if ($is_fk) {
                            ...
                            if (!$fk_col || !($fk_col->{$index}) || !($fk_col->{$index} eq $fks1)) {
                                ...
                                $self->{temporary_indexes}{$rc_index_name} = $index_part;
                                debug(3, "Added temporary index $rc_index_name for INDEX's field $index_part ".
                                      "in drop index condition because there is FK for this field in SECOND table ".
        ...
        for my $index (keys %$indices2) {
            ...
            if ($table2->isa_fk($index)) {
                ...
                if (!$table1->isa_fk($index)) {
                    debug(3, "index '$index` is not FK in first table, ".
                          "so it will be automatically added after FK creation");
                    $is_fk = 1;
                }
                else {
                    ...
                    if ($cmp_fk1 eq $cmp_fk2) {
                        ...
                        $is_fk = 1;
                        debug(3, "FKs are not identically in tables, ".
                              "so index '$index' will be automatically created after FK recreation");
                    }
                }
            }
                    ...
                    for my $dk (keys %$dropped_columns) {
                        ...
                        for my $dropped_column_fk (keys %$dropped_column_fks) {
                            if ($dropped_column_fk eq $fk) {
                                # column which this fk references was dropped, so now it's 
                                # referenced to another column (may be new column, we dont need to know it)
